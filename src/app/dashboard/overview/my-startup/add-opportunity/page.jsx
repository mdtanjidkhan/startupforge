...
//   const { data: session } = authClient.useSession(); // ✅ কারেন্ট সেশন ডাটা নেওয়া
...
//     // 💥 ক্রুসিয়াল ফিক্স: ফর্ম ডাটার সাথে ফাউন্ডারের সেশন ইমেইল পাঠানো হচ্ছে
...
//       founder_email: session?.user?.email || "founder@example.com", 
//     };

//     try {
      
//       const checkRes = await fetch(`http://localhost:5000/api/my-opportunities?email=${session?.user?.email}`);
//       const existingOpps = await checkRes.json();
//       if (existingOpps.length >= 3) {
//         toast.error("Free limit reached! Redirecting to Stripe premium checkout...", {
//           style: { borderRadius: '12px', background: '#1f2937', color: '#fff' }
//         });
        
//         // এখানে আপনার স্ট্রাইপ পেমেন্ট রাউট হিট হবে
//         // const stripeRes = await fetch("http://localhost:5000/api/create-checkout-session", { method: "POST" });
    ...
    <div className="max-w-3xl mx-auto space-y-6 px-4 py-8 md:p-0">
      ...
      <Card className="shadow-md dark:shadow-xl border border-gray-100 dark:border-slate-800 bg-white dark:bg-[#111827] rounded-2xl transition-colors duration-200">
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          {/* ২. Required Skills */}
          <div className="flex flex-col gap-2">
            <label className="text-sm font-semibold text-gray-700 dark:text-slate-300">Required Skills (Comma separated)</label>
            <Input
              isRequired
              type="text"
              placeholder="e.g., React, Node.js, Tailwind CSS"
